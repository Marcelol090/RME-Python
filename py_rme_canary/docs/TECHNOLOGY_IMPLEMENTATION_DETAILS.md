...
- ✓ DoorBrush (MVP / virtual): ferramentas na paleta Terrain (Normal/Locked/Magic/Quest/Window/Hatch). Clique/drag coloca uma porta “default” (via `items.xml`) e, se já houver uma porta no tile, alterna open/closed (usa door-pairs). Alt remove a porta do topo. 1 undo por gesto.
...
- Brushes: DoorBrush (MVP / virtual): paleta Terrain expõe ferramentas de porta por tipo (normal/locked/magic/quest/window/hatch); clique/drag coloca uma porta default ou alterna open/closed se já existir; Alt remove; 1 undo por gesto.
...
- DoorBrush (MVP): drag/smear (place/toggle/remove) com 1 undo por gesto.
