...
- [x] Preferences Runtime Contract (legacy parity) → `File > Preferences...` routes to modern `SettingsDialog` with structured apply/persist pipeline (`settings_applied` -> `_apply_settings`) and safe fallback to legacy dialog in `vis_layer/ui/main_window/qt_map_editor_file.py`.
