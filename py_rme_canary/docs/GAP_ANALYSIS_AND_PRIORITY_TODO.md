...
- **Critical Missing Area:** Specialized Brushes (House, Spawn, Waypoint) are mostly "Virtual" (stubs in palette) rather than fully implemented logic.
- **Critical UI Missing:** Property Inspectors (for Items/House/Spawns) and advanced Selection Modes.
...
- [ ] **Live Collaboration (Real-Time):**
    ...
    - **Task:** Finish the `NetworkedActionQueue`.
        - [ ] State Synchronization (Initial map sync).
        - [ ] Cursor Broadcasting (See other users' mouse).
        - [ ] Chat/Log Interface.
- [x] **Minimap Nav & Export:**
    - [x] **Interactive Minimap:** Click to navigate (currently basic).
    - [ ] **Full Export:** Export huge maps to PNG (tiled if necessary).
...
## 4. Next Action
**Recommendation:** Start with **P0 - Implement HouseBrush and SpawnBrush**. These are critical for map makers (RPGs rely on houses and monsters).
