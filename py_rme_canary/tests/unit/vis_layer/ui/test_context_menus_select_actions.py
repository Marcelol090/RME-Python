...
def test_tile_context_menu_disables_copy_cut_delete_without_selection(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(menus_module, "ContextMenuBuilder", _FakeBuilder)
    callbacks = {
        "copy": lambda: None,
        "cut": lambda: None,
    ...
    builder = _FakeBuilder.last
    assert builder is not None
    states = {entry[1]: entry[2] for entry in builder.actions if entry[0] == "action"}
    assert states["Copy"] is False
    assert states["Cut"] is False
    assert states["Delete"] is False
    assert states["Paste"] is True


def test_tile_context_menu_enables_copy_cut_delete_with_selection(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(menus_module, "ContextMenuBuilder", _FakeBuilder)
    callbacks = {
        "copy": lambda: None,
        "cut": lambda: None,
    ...
    assert builder is not None
    states = {entry[1]: entry[2] for entry in builder.actions if entry[0] == "action"}
    assert states["Copy"] is True
    assert states["Cut"] is True
    assert states["Delete"] is True
    assert states["Paste"] is True
...
def test_tile_context_menu_disables_browse_field_when_capability_gate_is_false(
    ...
    browse_entries = [entry for entry in builder.actions if entry[0] == "action" and entry[1] == "Browse Field"]
    assert len(browse_entries) == 1
    assert browse_entries[0][2] is False


def test_tile_context_menu_disables_properties_on_empty_tile(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(menus_module, "ContextMenuBuilder", _FakeBuilder)
    callbacks = {
        "copy": lambda: None,
        "cut": lambda: None,
    ...
    menu = menus_module.TileContextMenu(None)
    menu.set_callbacks(callbacks)
    menu.show_for_tile(tile=Tile(x=1, y=2, z=7, ground=None, items=[]), has_selection=False)

    builder = _FakeBuilder.last
    assert builder is not None
    prop_entries = [entry for entry in builder.actions if entry[0] == "action" and entry[1] == "Properties..."]
    assert len(prop_entries) == 1
    assert prop_entries[0][2] is False


def test_tile_context_menu_enables_properties_when_tile_has_payload(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(menus_module, "ContextMenuBuilder", _FakeBuilder)
    callbacks = {
        "copy": lambda: None,
        "cut": lambda: None,
    ...
    prop_entries = [entry for entry in builder.actions if entry[0] == "action" and entry[1] == "Properties..."]
    assert len(prop_entries) == 1
    assert prop_entries[0][2] is True
...
def test_tile_context_menu_keeps_legacy_order_select_before_properties_and_browse(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(menus_module, "ContextMenuBuilder", _FakeBuilder)
    callbacks = {
    ...
    assert labels.index("Select RAW") < labels.index("Properties...")
    assert labels.index("Properties...") < labels.index("Browse Field")
