...
class MinimapExporter:
    ...
    def __init__(self, *, items_db: "ItemsXML | None" = None) -> None:
    ...
    def _get_tile_minimap_color(self, tile: Any) -> int:
        ...
        if self.items_db:
            it = self.items_db.get(int(sid))
            if it:
                val = it.attributes.get("minimapColor")
                if val is not None:
                    try:
                        return int(val)
                    except ValueError:
...
def get_minimap_exporter(*, items_db: "ItemsXML | None" = None) -> MinimapExporter:
    return MinimapExporter(items_db=items_db)
