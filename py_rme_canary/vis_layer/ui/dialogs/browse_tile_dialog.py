...
class BrowseTileDialog(QDialog):
    ...
    def _load_items(self) -> None:
        ...
        if self.tile.ground:
            ...
            entry = ItemStackEntry(item=self.tile.ground, index=0, is_ground=True, sprite_pixmap=sprite)
        ...
        for i, item in enumerate(self.tile.items):
            ...
            entry = ItemStackEntry(item=item, index=i + 1, is_ground=False, sprite_pixmap=sprite)
            self._stack_entries.append(entry)
    ...
    def _on_move_up(self) -> None:
        ...
        if current_idx > 1:  # Can't move above ground
            self._stack_entries[current_idx], self._stack_entries[current_idx - 1] = (
                self._stack_entries[current_idx - 1],
                self._stack_entries[current_idx],
            )
            self._refresh_list()
