...
class QtMapEditorFileMixin:
    ...
    def _open_otbm(self: "QtMapEditor") -> None:
        ...
        self.items_db = loader.last_items_db
    ...
    def _export_otmm(self: "QtMapEditor") -> None:
        if not self.map:
            return
        path, _ = QFileDialog.getSaveFileName(self, "Export OTMM Minimap", "", "OTMM Files (*.otmm);;All Files (*)")
        if not path:
            return
        if not path.lower().endswith(".otmm"):
            path += ".otmm"

        try:
            # Pass items_db to the exporter
            exporter = get_minimap_exporter(items_db=self.items_db)
            z_level = int(self.viewport.z)
            exporter.export(self.map, path, z_level=z_level)
            QMessageBox.information(
                self, "Export Complete", f"Minimap exported to {path}\nLevel: {z_level}"
            )
        except Exception as e:
