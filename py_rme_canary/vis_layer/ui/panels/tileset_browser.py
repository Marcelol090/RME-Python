...
class TilesetBrowser(QWidget):
    ...
    def set_data(
        self,
        categories: list[TilesetCategory],
        brushes: list[TilesetBrush]
    ) -> None:
        """Set tileset data.
        ...
        for cat in categories:
            cat.brush_count = sum(
                1 for b in brushes if b.category_id == cat.id
            )
    ...
    def _on_search_changed(self, text: str) -> None:
        ...
        filtered = [
            b for b in self._brushes
            if text_lower in b.name.lower() or text_lower in str(b.brush_id)
        ]
