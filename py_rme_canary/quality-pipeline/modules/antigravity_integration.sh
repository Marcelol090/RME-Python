...
run_antigravity_integration() {
  ...
  # Run analysis
  local output="$REPORT_DIR/antigravity_analysis.json"
  local mode="${QUALITY_ANTIGRAVITY_MODE:-refactor}"  # refactor|review|security
  ...
  if python3 "$WORKERS_DIR/antigravity_client.py" \
    ...
    # Parse results
    local high_priority=$(jq '[.[] | select(.priority == "high")] | length' "$output" 2>/dev/null || echo 0)
    local total=$(jq 'length' "$output" 2>/dev/null || echo 0)
